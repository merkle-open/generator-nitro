...
describe('Hbs Pattern Helper', () => {
	it('returns an error message if module is unknown', () => {
		expect(hbs.handlebars.helpers.pattern('nonexistent')).toMatch(
			`<p class="nitro-msg nitro-msg--error">Pattern \`nonexistent\` with template file \`nonexistent.${config.get(
				'nitro.viewFileExtension'
			)}\` not found in folder \`nonexistent\`.</p>`
		);
	});

	it('returns an error message if module with specific type is unknown', () => {
		expect(hbs.handlebars.helpers.pattern('example', { hash: { template: 'example', type: 'organism' } })).toMatch(
			`<p class="nitro-msg nitro-msg--error">Pattern \`example\` within pattern type \`organism\` with template file \`example.${config.get(
				'nitro.viewFileExtension'
			)}\` not found in folder \`example\`.</p>`
		);
	});

	it('returns an error message if module type is unknown', () => {
		expect(
			hbs.handlebars.helpers.pattern('example', { hash: { template: 'example', type: 'nonexistent' } })
		).toMatch(
			`<p class="nitro-msg nitro-msg--error">Pattern type \`nonexistent\` not found in pattern config.</p>`
		);
	});
	...
	it('returns the specific pattern found with the provided name AND type parameter', () => {
		// eslint-disable-next-line
		expect(hbs.handlebars.helpers.pattern('example', { hash: { template: 'example', type: 'molecule' } })).toMatch(
			'<div>m</div>'
		);
	});
});
