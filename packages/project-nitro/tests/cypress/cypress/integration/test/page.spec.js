...
context('Test Page Tests', () => {
	...
	describe('Content', () => {
		it('headline 1 contains text', () => {
			cy.get('.t-ex__headline:first')
				.contains('Ex Module (data: ex.json)');
		});
		it('headline 2 contains text', () => {
			cy.get('.t-ex--blue > .t-ex__headline')
				.contains('Ex Module with Skin Blue (data: ex-blue.json)');
		});
		it('headline 3 contains text', () => {
			cy.get('[data-t-decorator="Template"] > .t-ex__headline')
				.contains('Ex Module with Decorator Template');
		});
	...
	describe('Translations', () => {
		it('text contains', () => {
			...
			cy.get('.t-t > :nth-child(4) > :nth-child(2)')
				.contains('test.text (t) = test.text (t)');

			cy.get('.t-t > :nth-child(8) > :nth-child(1)')
				.contains('user.name: my name');

			cy.get('.t-t > :nth-child(8) > :nth-child(2)')
				.contains('user.email: me@test.com');

			cy.get('p').contains('content string');

			cy.get('.t-a > :nth-child(1)')
				.contains('ffffffrom b');

			cy.get('.t-a > :nth-child(6)')
				.contains('from viewwwwwww');

			cy.get('.col-md-8 > :nth-child(7)')
				.contains('[Partial] pageTitle (view) = pageTitle (view)');
		});
	});
});
