...
module.exports = generators.Base.extend({
	...
	constructor: function () {
		...
		this.types = _.map(this.cfg.nitro.patterns, (value, key) => {
			return key;
		});
	...
	prompting() {
		...
		this.log(yosay(
			'Let me help you to create your pattern…'
		));

		return this.prompt([
			{
				name: 'name',
				message: 'What\'s the name of your pattern?',
				default: this.name,
				validate: function validateString(value) {
					if (!_.isString(value) || _.isEmpty(value)) {
						return 'Pattern name has to be a valid string';
					}
					if (/^[0-9]/.test(value)) {
						return 'Pattern name must not start with a Number';
					}
					return true;
				}
			},
			...
			},
			{
				...
				validate: function validateString(value) {
					if (_.isString(value) && (/^[0-9]/).test(value)) {
						return 'Pattern decorator must not start with a Number';
					}
					return true;
				}
			}
	...
	writing: {
		app() {
			...
			const pattern = this.cfg.nitro.patterns[this.options.type];
			...
			const replacements = {
				...
				pattern: {
					...
					prefix: pattern.pattern_prefix || null, // CSS class prefix, eg. m
